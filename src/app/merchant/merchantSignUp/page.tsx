...
const MerchantSignUpPage: React.FC = () => {
    ...
    const { isSessionExist, servicesCategoryList } = useContext(DataContext);
    ...
    async function signUpUser() {
        ...
        if (!fullName || !email || !userName || !password || !businessName || !invitedBy || !serviceCategory) {
            setsignUpresponseMessage("All fields are required");
            setloading(false)
            return;
        }

        const data = { fullName, email, userName, password, businessName, invitedBy, websiteLink, serviceCategory };

        const response = await axios.post('/api/merchantsAPIs/merchantSignUpAPI', data);
    ...
    async function verifyUser() {
        ...
        const response = await axios.put('/api/merchantsAPIs/merchantSignUpAPI', data);
    ...
    }, [fullName, email, userName, password, businessName, invitedBy])
    ...
    useEffect(() => {
        const url = `https://www.blyt.world/merchant/${userName}/${businessName}`;
        const modifiedUrl = url.replace(/\s+/g, '-');
        setwebsiteLink(modifiedUrl)
    }, [userName, businessName, websiteLink])
                ...
                <section className="merchant-sign-up-section h-screen w-full flex items-center px-4 md:max-w-screen-lg max-w-xs m-auto">
                    <div className="max-width flex gap-50 w-full justify-evenly gap-2">
                        ...
                        <section className="right-side-section flex flex-col gap-5 w-full">
                            ...
                            <InputComonentForSignInUpPage
                                title="company name"
                                image={<SignInUpBriefcaseIcon1 width={20} height={20} customCSS={''} />}
                                inputType="name"
                                value={businessName}
                                onChange={(e: { target: { value: React.SetStateAction<string>; }; }) => setbusinessName(e.target.value)} />
                            ...
                            <section className="inputs-2 flex sm:flex-row flex-col justify-between gap-10 relative w-full">
                                ...
                                <div className="absolute capitalize font-bold bg-[#F5F5F5] px-2 -top-3 left-5">service category</div>
                                <select className="w-full border border-black rounded-lg pl-8 py-3 bg-transparent default:" value={serviceCategory} onChange={(e) => setserviceCategory(e.target.value)}>
                                    <option value="">Select A Category</option>
                                    {servicesCategoryList.map((item: ServiceCategoriesList) => (
