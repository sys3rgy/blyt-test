...
import { PackageListData } from '@/interfaces/MerchantDashboardInterface';
...
const EditAPackagePage: React.FC<{ params: Params }> = ({ params }) => {

    const { userName, packageId } = params;
    ...
    async function checkMerchantSession() {
        ...
        const response = await axios.get(`/api/merchantsAPIs/merchantAccountDataFetchUpdateAPI?userName=${userName}&packageId=${packageId}`)
        ...
        const data = response.data.fetchPackageData;
        setpackageName(data.packageName || '');
        setpackagePrice(data.packagePrice || '');
        setpackageDescription(data.packageDescription || '');

        const newImagesUrls = data.packageImages.map((url: any, index: any) => ({
            url: url,
            id: generateUniqueId(), // Generate unique ID for each image
            order: index // Set the order based on index
        }));

        setpackageImages(newImagesUrls || []);
    ...
    async function uploadPackageToDB() {
        ...
        const response = await axios.put('/api/merchantsAPIs/merchantPackageAddUpdateAPI', formData, {
            headers: {
                    ...
                    <section className="add-a-package-form-section pb-10">
                        <section className="max-width max-w-screen-lg m-auto px-4">
                            ...
                            <button className='flex m-auto' onClick={uploadPackageToDB}>
                                <ButtonComponent1
                                    middleSide="update package"
                                    rightSide={<PlusIconInCircle2 width={20} height={20} customCSS={''} />}
                                    customButtonCSS="uppercase flex m-auto text-white font-bold gap-4 rounded-lg drop-shadow-lg items-center bg-[#6D6E72] !px-10 !py-5 w-fit" />
                            </button>
...
export default EditAPackagePage