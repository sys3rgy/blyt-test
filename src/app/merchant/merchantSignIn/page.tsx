...
import { EyeIcon1, LoadingGif, SignInAndSignUpIcon1, SignInUpPasswordIcon1, SignInUpUserIcon1 } from '@/images/ImagesExport';
import axios from 'axios';
import { useRouter } from 'next/navigation';
import React, { useContext, useEffect, useState } from 'react';

const MerchantSignInPage: React.FC = () => {
    ...
    async function signIn() {
        ...
        const response = await axios.post('/api/merchantsAPIs/merchantSignInAPI', data)
    ...
    async function verifyUserWhoseAccAreVerified() {
        ...
        const response = await axios.put('/api/merchantsAPIs/merchantSignInAPI', data);
        const statusCode = response.data.statusCode;
        const message = response.data.message;
                ...
                <section className="merchant-sign-up-verify-section h-screen w-full flex items-center px-4 md:max-w-screen-lg max-w-xs m-auto">
                    <div className="max-width flex gap-50 w-full justify-evenly gap-2">
                        <section className="right-side-section flex flex-col gap-5 ">
                            ...
                            <div className='cursor-pointer w-1/2 flex justify-center mx-auto' onClick={verifyUserWhoseAccNotVerified}>
                                <ButtonComponent1
                                    leftSide={undefined}
                                    middleSide={'verify'}
                                    rightSide={undefined}
                ...
                <section className="merchant-sign-in-verify-section h-screen w-full flex items-center px-4 md:max-w-screen-lg max-w-xs m-auto">
                    <div className="max-width flex gap-50 w-full justify-evenly gap-2">
                        <section className="right-side-section flex flex-col gap-5 ">
                            ...
                            <div className='cursor-pointer w-1/2 flex justify-center mx-auto' onClick={verifyUserWhoseAccAreVerified}>
                                <ButtonComponent1
                                    leftSide={undefined}
                                    middleSide={'verify'}
                                    rightSide={undefined}
...
export default MerchantSignInPage;