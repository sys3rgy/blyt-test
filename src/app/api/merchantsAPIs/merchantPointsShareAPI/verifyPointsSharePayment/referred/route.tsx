...
export async function GET(request: NextRequest) {
    ...
    if (paymentStatus === 1) {
        ...
        const getBuyerId = await MerchantSharePointsModel.findOneAndUpdate({ uniqueId: referenceCode }, { isPaymentDone: true, isPointsTransferred: true }).select('orderId merchantId referredUserName pointsToShare isPointsTransferred').lean();
        ...
        const getBuyerUsername = await AccountsModel.findOneAndUpdate({ userName: getBuyerId.referredUserName }, { $inc: { blytPoints: getBuyerId.pointsToShare } }).select('userName').lean();
        ...
        await OrdersListModel.updateOne({ _id: getBuyerId.orderId }, { isPointsSharedToReferred: true, pointsSharedToReferred: getBuyerId.pointsToShare });
