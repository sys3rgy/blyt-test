...
if (
	process.env.NEXT_PRIVATE_BCRYPT_SALT_ROUNDS === undefined ||
	process.env.NEXT_PRIVATE_BCRYPT_SALT_ROUNDS.length === 0 ||
	Number.isNaN(Number(process.env.NEXT_PRIVATE_BCRYPT_SALT_ROUNDS))
) {
...
import AdminAccountsModel from "@/models/AdminAccountsModel";
...
export async function POST(request: Request) {
	...
	//* Find if user exist
	const findUser = await AdminAccountsModel.findOne({ userName }).lean();
...
export async function PUT(request: Request) {
	...
	if (compareOTP === false) {
		...
		//! Find user id
		const getUserAccountId = await AdminAccountsModel.findOne({ userName }).select("_id userFullName").lean();
